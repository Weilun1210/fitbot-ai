"""FitBot Dialogflow ES exercise webhook (v5.8).

The webhook answers only from ``megaGymDataset.csv``. It supports strict
...
DEFAULT_DATASET_CANDIDATES = (
    HERE / "megaGymDataset.csv",
    HERE.parents[2] / "02 data" / "raw" / "megaGymDataset.csv",
)

# Canonical values are the exact values present in megaGymDataset.csv.
BODY_PART_VALUES = (
...
def dataset_path() -> Path:
    ...
    for candidate in DEFAULT_DATASET_CANDIDATES:
        if candidate.is_file():
            return candidate
    raise FileNotFoundError("megaGymDataset.csv was not found")


def load_exercises(path: Path | None = None) -> list[dict[str, str]]:
    """Load all usable exercise rows from the shared CSV."""
    ...
    with source.open("r", encoding="utf-8-sig", newline="") as stream:
        ...
        return [
            ...
            for row in reader
            if row.get("Title") and row.get("BodyPart") and row.get("Equipment")
            and row.get("Level") and row.get("Type")
        ]
